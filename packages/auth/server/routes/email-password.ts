...
export const emailPasswordRoute = new Hono<HonoAuthContext>()
  ...
  /**
   * SSO auto-login endpoint for JustX "Meus Documentos" redirect.
   * Token is a short-lived encrypted payload generated by /api/internal/justx/generate-login-url.
   */
  .get('/sso', async (c) => {
    const token = c.req.query('token') ?? '';
    const redirect = c.req.query('redirect') ?? '/documents';

    // Validate redirect is a safe relative path
    const safeRedirect =
      redirect.startsWith('/') && !redirect.startsWith('//') ? redirect : '/documents';

    const SIGNIN_ERROR_URL = '/signin?error=session_expired';

    if (!token) {
      return c.redirect(SIGNIN_ERROR_URL);
    }

    try {
      const decrypted = decryptSecondaryData(token);
      if (!decrypted) {
        return c.redirect(SIGNIN_ERROR_URL);
      }

      const { userId } = JSON.parse(decrypted) as { userId: number };
